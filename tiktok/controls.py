...
class TikTokAutomation:
    def __init__(self, device: u2.Device):
        ...
        self.status_bar_h = device(resourceId="com.android.systemui:id/status_bar").info['bounds']['bottom']
    
    #* 抽离公共方法
    def _click_button(self, class_name: str, description: str = None, descriptionMatches: str = None, text: str = None, index: int = None):
        """
        通用按钮点击方法
        ...
        try:
            ...
            if button.exists:
                button.click()
                logger.success(f"成功点击 {description or descriptionMatches or text} 按钮")
                return True, f"点击{description or descriptionMatches or text}按钮成功"
            else:
                logger.success(f"未找到 {description or descriptionMatches or text} 按钮")
    ...
    def judgment_page(self):
        ...
        try:
            ...
            search_conditions = [
                {"title": "搜索页面", "condition": {"resourceId": "com.zhiliaoapp.musically:id/fho", "index": 2}},  # 第一个条件
                {"title": "首页", "condition": {"className": "android.widget.LinearLayout","description":"推荐"}},  # 第二个条件（备用）
                {"title": "搜索框", "condition": {"text": "搜索", "className": "android.widget.TextView"}},  # 第三个条件（备用）
                # 可以继续添加其他条件和标题
            ]
...
#*  视频页面操作
    ...
    def favorite_video(self):
        """收藏视频"""
        return self._click_button("android.widget.Button", description="将此视频添加到或移出收藏。")

    def share_video(self):
        """分享视频"""
        return self._click_button("android.widget.Button", descriptionMatches="^分享视频。.*")
